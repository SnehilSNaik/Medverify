...
import { 
  ...
  AlertCircle, GraduationCap, Briefcase, FileCheck, HeartPulse
} from 'lucide-react';
...
const StudentSignup = () => {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-pink-50/60 via-white to-rose-50/50 relative overflow-hidden p-4 py-8">
      <div className="absolute top-[-10%] left-[-10%] w-[500px] h-[500px] bg-pink-200/40 opacity-50 rounded-full blur-[120px] pointer-events-none"></div>
      ...
      <div className="w-full max-w-4xl flex flex-col md:flex-row bg-white rounded-3xl overflow-hidden z-10 border border-pink-100 shadow-[0_20px_60px_-15px_rgba(236,72,153,0.18)]">
        
        {/* Left Branding Side */}
        <div className="hidden md:flex md:w-5/12 bg-gradient-to-br from-pink-500 via-rose-500 to-pink-600 text-white flex-col justify-between p-8 relative overflow-hidden">
          ...
          <div className="my-auto py-8 relative z-10 flex flex-col items-center text-center">
            ...
            <p className="text-[10px] uppercase tracking-[0.2em] text-pink-100 font-bold mb-3">Personal access</p>
            <h2 className="text-2xl font-black text-white mb-2 leading-tight">Student & Employee Vault</h2>
            <p className="text-pink-100 text-xs font-medium">
              Personal Medical Records Portal
            </p>
          </div>

          <div className="text-[11px] text-pink-100/80 border-t border-white/20 pt-3 relative z-10 text-center font-medium">
            MedVerify Platform
          </div>
        </div>
        ...
        <div className="w-full md:w-7/12 p-6 md:p-8 flex flex-col justify-center bg-white overflow-y-auto max-h-[90vh]">
          <div className="md:hidden flex items-center justify-center mb-4 gap-2">
            <div className="p-2 bg-gradient-to-tr from-pink-500 to-rose-500 rounded-xl text-white">
              <HeartPulse size={22} />
            </div>
          ...
          <div className="mb-5">
            <h2 className="text-xl font-extrabold text-slate-800 tracking-tight">Student / Employee Signup</h2>
            <p className="text-slate-500 text-xs font-medium">Create your personal health vault to manage certificates</p>
          </div>
          ...
          <form onSubmit={handleSignup} className="space-y-3.5">
            <div>
              ...
              <div className="relative">
                ...
                <input type="text" className="form-input pl-9" placeholder="e.g. Alex Johnson" value={fullName} onChange={(e) => setFullName(e.target.value)} />
              </div>
            </div>

            <div>
              <label className="form-label">College / University / Company</label>
              <div className="relative">
                <span className="absolute inset-y-0 left-0 pl-3 flex items-center text-slate-400"><Building2 size={15} /></span>
                <input type="text" className="form-input pl-9" placeholder="e.g. Oxford University / Tech Corp" value={organizationName} onChange={(e) => setOrganizationName(e.target.value)} />
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <div>
                ...
                <div className="relative">
                  <span className="absolute inset-y-0 left-0 pl-3 flex items-center text-slate-400"><User size={15} /></span>
                  <input type="text" className="form-input pl-9" placeholder="e.g. alex_j" value={username} onChange={(e) => setUsername(e.target.value)} required />
                </div>
              </div>
              <div>
                ...
                <div className="relative">
                  ...
                  <input type="email" className="form-input pl-9" placeholder="alex@email.com" value={email} onChange={(e) => setEmail(e.target.value)} required />
                </div>
              </div>
            </div>
            ...
            <button type="submit" className="btn btn-primary w-full py-3 mt-2 shadow-[0_6px_20px_rgba(236,72,153,0.35)]" disabled={loading}>
              {loading ? <Loader2 className="animate-spin" size={16} /> : (
                <>
                  <GraduationCap size={14} />
                  <span>Create Student / Employee Account</span>
                </>
              )}
            </button>
          </form>

          <div className="mt-4 pt-4 border-t border-pink-100 flex items-center justify-between text-xs font-semibold">
            <span className="text-slate-500">Already registered?</span>
            <Link to="/login" className="text-pink-600 hover:text-pink-800 underline transition-colors">
              Sign In Here →
            </Link>
          </div>

          <div className="mt-3 pt-3 border-t border-pink-50 text-center">
            <p className="text-[11px] text-slate-400 mb-1.5 font-medium">Not a student or employee?</p>
